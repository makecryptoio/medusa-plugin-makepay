...
async function connectSandboxOAuth(
  ...
  if (reconnect || !(await disconnect.isVisible())) {
    ...
    if (!callbackResponse) {
      ...
      const connection = reconciled.connection;
      if (
        connection?.connected !== true ||
        connection?.status !== "connected" ||
        connection?.webhook?.status !== "healthy" ||
        connection?.company_id !== sandboxCompanyId ||
        JSON.stringify([...(connection?.scopes || [])].sort()) !==
          JSON.stringify([...expectedScopes].sort())
      ) {
        throw new Error(
          "MakePay OAuth callback response was lost and the connection could not be reconciled.",
        );
      }
      await gotoDisposableTunnel(
        page,
  ...
  await expect
    .poll(async () => {
      ...
      return {
        ...
        webhook: connection.connection?.webhook?.status,
      };
    })
    .toEqual({ connected: true, status: "connected", webhook: "healthy" });
  expect(connection.connection?.company_id).toBe(sandboxCompanyId);
  expect([...(connection.connection?.scopes || [])].sort()).toEqual(
    [...expectedScopes].sort(),
  );
